...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the response using every rubric criterion below. "
        ...
        try:
            ...
            values = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            lines.append(
                f"| F{index:03d} | {safe(failure.failure_type or 'unknown')} | "
                f"{safe(self.find_root_cause(failure))} | {safe(suggestion)} | Open |"
            )
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": "Add grounding checks and require factual claims to be supported by retrieved context",
            "irrelevant": "Clarify the answer prompt and add intent-routing examples for common question types",
            "incomplete": "Improve retrieval coverage and prompt the generator to address every required answer point",
            "off_topic": "Strengthen intent detection and add an off-topic response guardrail",
            "refusal": "Review refusal rules and add examples of safe in-scope questions that must be answered",
            "unknown": "Inspect uncategorized failures and add a precise failure label before the next run",
        }
        ...
        general_actions = [
            "Add failed cases to the regression dataset and evaluate every prompt or retrieval change",
            "Review low-scoring traces to separate retrieval errors from generation errors",
            "Track per-category metrics so the highest-impact failure cluster is fixed first",
        ]
